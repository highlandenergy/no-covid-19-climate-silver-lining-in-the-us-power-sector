...
def oil(num_inducing=50, max_iters=100, kernel=None, optimize=True, plot=True):
    ...
    # Create GP model
    m = GPy_1_0_5.models.SparseGPClassification(X, Y, kernel=kernel, num_inducing=num_inducing)
    m.Ytest = Ytest
    ...
    GPy_1_0_5.util.classification.conf_matrix(probs, Ytest)
    return m
...
def toy_linear_1d_classification(seed=default_seed, optimize=True, plot=True):
    ...
    # Model definition
    m = GPy_1_0_5.models.GPClassification(data['X'], Y)
...
def toy_linear_1d_classification_laplace(seed=default_seed, optimize=True, plot=True):
    ...
    kernel = GPy_1_0_5.kern.RBF(1)

    # Model definition
    m = GPy_1_0_5.core.GP(data['X'], Y, kernel=kernel, likelihood=likelihood, inference_method=laplace_inf)
...
def sparse_toy_linear_1d_classification(num_inducing=10, seed=default_seed, optimize=True, plot=True):
    ...
    # Model definition
    m = GPy_1_0_5.models.SparseGPClassification(data['X'], Y, num_inducing=num_inducing)
    m['.*len'] = 4.
...
def sparse_toy_linear_1d_classification_uncertain_input(num_inducing=10, seed=default_seed, optimize=True, plot=True):
    ...
    # Model definition
    m = GPy_1_0_5.models.SparseGPClassificationUncertainInput(X, X_var, Y, num_inducing=num_inducing)
    m['.*len'] = 4.
...
def toy_heaviside(seed=default_seed, max_iters=100, optimize=True, plot=True):
    ...
    # Model definition
    kernel = GPy_1_0_5.kern.RBF(1)
    likelihood = GPy_1_0_5.likelihoods.Bernoulli(gp_link=GPy_1_0_5.likelihoods.link_functions.Heaviside())
    ep = GPy_1_0_5.inference.latent_function_inference.expectation_propagation.EP()
    m = GPy_1_0_5.core.GP(X=data['X'], Y=Y, kernel=kernel, likelihood=likelihood, inference_method=ep, name='gp_classification_heaviside')
    #m = GPy_1_0_5.models.GPClassification(data['X'], likelihood=likelihood)
...
def crescent_data(model_type='Full', num_inducing=10, seed=default_seed, kernel=None, optimize=True, plot=True):
    ...
    :type kernel: a GPy_1_0_5 kernel
    """
    try:import pods
    except ImportError:print('pods unavailable, see https://github.com/sods/ods for example datasets')
    ...
    if model_type == 'Full':
        m = GPy_1_0_5.models.GPClassification(data['X'], Y, kernel=kernel)

    elif model_type == 'DTC':
        m = GPy_1_0_5.models.SparseGPClassification(data['X'], Y, kernel=kernel, num_inducing=num_inducing)
        m['.*len'] = 10.

    elif model_type == 'FITC':
        m = GPy_1_0_5.models.FITCClassification(data['X'], Y, kernel=kernel, num_inducing=num_inducing)
        m['.*len'] = 3.
    if optimize:
        m.optimize(messages=1)
