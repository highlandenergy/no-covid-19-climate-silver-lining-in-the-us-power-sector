...
import GPy_1_0_5
import numpy as np
from GPy_1_0_5.util import datasets
try:
...
def student_t_approx(optimize=True, plot=True):
    ...
    # Kernel object
    kernel1 = GPy_1_0_5.kern.RBF(X.shape[1]) + GPy_1_0_5.kern.White(X.shape[1])
    kernel2 = GPy_1_0_5.kern.RBF(X.shape[1]) + GPy_1_0_5.kern.White(X.shape[1])
    kernel3 = GPy_1_0_5.kern.RBF(X.shape[1]) + GPy_1_0_5.kern.White(X.shape[1])
    kernel4 = GPy_1_0_5.kern.RBF(X.shape[1]) + GPy_1_0_5.kern.White(X.shape[1])

    #Gaussian GP model on clean data
    m1 = GPy_1_0_5.models.GPRegression(X, Y.copy(), kernel=kernel1)
    # optimize
    m1['.*white'].constrain_fixed(1e-5)
    m1.randomize()

    #Gaussian GP model on corrupt data
    m2 = GPy_1_0_5.models.GPRegression(X, Yc.copy(), kernel=kernel2)
    m2['.*white'].constrain_fixed(1e-5)
    m2.randomize()

    #Student t GP model on clean data
    t_distribution = GPy_1_0_5.likelihoods.StudentT(deg_free=deg_free, sigma2=edited_real_sd)
    laplace_inf = GPy_1_0_5.inference.latent_function_inference.Laplace()
    m3 = GPy_1_0_5.core.GP(X, Y.copy(), kernel3, likelihood=t_distribution, inference_method=laplace_inf)
    ...
    t_distribution = GPy_1_0_5.likelihoods.StudentT(deg_free=deg_free, sigma2=edited_real_sd)
    laplace_inf = GPy_1_0_5.inference.latent_function_inference.Laplace()
    m4 = GPy_1_0_5.core.GP(X, Yc.copy(), kernel4, likelihood=t_distribution, inference_method=laplace_inf)
...
def boston_example(optimize=True, plot=True):
    ...
    for n, (train, test) in enumerate(kf):
        ...
        rbf_len = 0.5
        data_axis_plot = 4
        kernelstu = GPy_1_0_5.kern.RBF(X.shape[1]) + GPy_1_0_5.kern.white(X.shape[1]) + GPy_1_0_5.kern.bias(X.shape[1])
        kernelgp = GPy_1_0_5.kern.RBF(X.shape[1]) + GPy_1_0_5.kern.white(X.shape[1]) + GPy_1_0_5.kern.bias(X.shape[1])
        ...
        #Gaussian GP
        print("Gauss GP")
        mgp = GPy_1_0_5.models.GPRegression(X_train.copy(), Y_train.copy(), kernel=kernelgp.copy())
        ...
        N, D = Y_train.shape
        g_distribution = GPy_1_0_5.likelihoods.noise_model_constructors.gaussian(variance=noise, N=N, D=D)
        g_likelihood = GPy_1_0_5.likelihoods.Laplace(Y_train.copy(), g_distribution)
        mg = GPy_1_0_5.models.GPRegression(X_train.copy(), Y_train.copy(), kernel=kernelstu.copy(), likelihood=g_likelihood)
        ...
        for stu_num, df in enumerate(degrees_freedoms):
            ...
            t_distribution = GPy_1_0_5.likelihoods.noise_model_constructors.student_t(deg_free=df, sigma2=noise)
            stu_t_likelihood = GPy_1_0_5.likelihoods.Laplace(Y_train.copy(), t_distribution)
            mstu_t = GPy_1_0_5.models.GPRegression(X_train.copy(), Y_train.copy(), kernel=kernelstu.copy(), likelihood=stu_t_likelihood)
