# Copyright (c) 2012-2014, GPy_1_0_5 authors (see AUTHORS.txt).
...
def olympic_marathon_men(optimize=True, plot=True):
    ...
    m = GPy_1_0_5.models.GPRegression(data['X'], data['Y'])
...
def coregionalization_toy(optimize=True, plot=True):
    ...
    m = GPy_1_0_5.models.GPCoregionalizedRegression(X_list=[X1,X2], Y_list=[Y1,Y2])

    if optimize:
        m.optimize('bfgs', max_iters=100)

    if plot:
        slices = GPy_1_0_5.util.multioutput.get_slices([X1,X2])
...
def coregionalization_sparse(optimize=True, plot=True):
    ...
    m = GPy_1_0_5.models.SparseGPCoregionalizedRegression(X_list=[X1,X2], Y_list=[Y1,Y2])
...
def epomeo_gpx(max_iters=200, optimize=True, plot=True):
    ...
    k1 = GPy_1_0_5.kern.RBF(1)
    k2 = GPy_1_0_5.kern.Coregionalize(output_dim=5, rank=5)
    k = k1**k2

    m = GPy_1_0_5.models.SparseGPRegression(t, Y, kernel=k, Z=Z, normalize_Y=True)
...
def multiple_optima(gene_number=937, resolution=80, model_restarts=10, seed=10000, max_iters=300, optimize=True, plot=True):
    ...
    lls = GPy_1_0_5.examples.regression._contour_data(data, length_scales, log_SNRs, GPy_1_0_5.kern.RBF)
    if plot:
    ...
    for i in range(0, model_restarts):
        # kern = GPy_1_0_5.kern.RBF(1, variance=np.random.exponential(1.), lengthscale=np.random.exponential(50.))
        kern = GPy_1_0_5.kern.RBF(1, variance=np.random.uniform(1e-3, 1), lengthscale=np.random.uniform(5, 50))

        m = GPy_1_0_5.models.GPRegression(data['X'], data['Y'], kernel=kern)
...
def _contour_data(data, length_scales, log_SNRs, kernel_call=GPy_1_0_5.kern.RBF):
    """
    ...
    model = GPy_1_0_5.models.GPRegression(data['X'], data['Y'], kernel=kernel)
...
def toy_poisson_rbf_1d_laplace(optimize=True, plot=True):
    ...
    f_true = np.random.multivariate_normal(np.zeros(x_len), GPy_1_0_5.kern.RBF(1).K(X))
    Y = np.array([np.random.poisson(np.exp(f)) for f in f_true])[:,None]

    kern = GPy_1_0_5.kern.RBF(1)
    poisson_lik = GPy_1_0_5.likelihoods.Poisson()
    laplace_inf = GPy_1_0_5.inference.latent_function_inference.Laplace()

    # create simple GP Model
    m = GPy_1_0_5.core.GP(X, Y, kernel=kern, likelihood=poisson_lik, inference_method=laplace_inf)
...
def toy_ARD(max_iters=1000, kernel_type='linear', num_samples=300, D=4, optimize=True, plot=True):
    ...
    if kernel_type == 'linear':
        kernel = GPy_1_0_5.kern.Linear(X.shape[1], ARD=1)
    elif kernel_type == 'rbf_inv':
        kernel = GPy_1_0_5.kern.RBF_inv(X.shape[1], ARD=1)
    else:
        kernel = GPy_1_0_5.kern.RBF(X.shape[1], ARD=1)
    kernel += GPy_1_0_5.kern.White(X.shape[1]) + GPy_1_0_5.kern.Bias(X.shape[1])
    m = GPy_1_0_5.models.GPRegression(X, Y, kernel)
    # len_prior = GPy_1_0_5.priors.inverse_gamma(1,18) # 1, 25
...
def toy_ARD_sparse(max_iters=1000, kernel_type='linear', num_samples=300, D=4, optimize=True, plot=True):
    ...
    if kernel_type == 'linear':
        ...
        kernel = GPy_1_0_5.kern.RBF(X.shape[1], ARD=1)
    #kernel += GPy_1_0_5.kern.Bias(X.shape[1])
    X_variance = np.ones(X.shape) * 0.5
    m = GPy_1_0_5.models.SparseGPRegression(X, Y, kernel, X_variance=X_variance)
    # len_prior = GPy_1_0_5.priors.inverse_gamma(1,18) # 1, 25
    # m.set_prior('.*lengthscale',len_prior)
...
def robot_wireless(max_iters=100, kernel=None, optimize=True, plot=True):
    ...
    # create simple GP Model
    m = GPy_1_0_5.models.GPRegression(data['Y'], data['X'], kernel=kernel)
...
def sparse_GP_regression_1D(num_samples=400, num_inducing=5, max_iters=100, optimize=True, plot=True, checkgrad=False):
    ...
    # construct kernel
    rbf = GPy_1_0_5.kern.RBF(1)
    # create simple GP Model
    m = GPy_1_0_5.models.SparseGPRegression(X, Y, kernel=rbf, num_inducing=num_inducing)
...
def uncertain_inputs_sparse_regression(max_iters=200, optimize=True, plot=True):
    ...
    # likelihood = GPy_1_0_5.likelihoods.Gaussian(Y)
    Z = np.random.uniform(-3., 3., (7, 1))

    k = GPy_1_0_5.kern.RBF(1)
    # create simple GP Model - no input uncertainty on this one
    m = GPy_1_0_5.models.SparseGPRegression(X, Y, kernel=k, Z=Z)
    ...
    m = GPy_1_0_5.models.SparseGPRegression(X, Y, kernel=GPy_1_0_5.kern.RBF(1), Z=Z, X_variance=S)
    if optimize:
        m.optimize('scg', messages=1, max_iters=max_iters)
    if plot:
        m.plot(ax=axes[1])
...
def simple_mean_function(max_iters=100, optimize=True, plot=True):
    ...
    k =GPy_1_0_5.kern.RBF(1)
    lik = GPy_1_0_5.likelihoods.Gaussian()
    m = GPy_1_0_5.core.GP(X, Y, kernel=k, likelihood=lik, mean_function=mf)
    if optimize:
        m.optimize(max_iters=max_iters)
    if plot:
...
def parametric_mean_function(max_iters=100, optimize=True, plot=True):
    ...
    mf = GPy_1_0_5.mappings.Linear(1,1)
