...
def build_likelihood(Y_list,noise_index,likelihoods_list=None):
    ...
    if likelihoods_list is None:
       likelihoods_list = [GPy_1_0_5.likelihoods.Gaussian(name="Gaussian_noise_%s" %j) for y,j in zip(Y_list,range(Ny))]
    else:
        assert len(likelihoods_list) == Ny
    #likelihood = GPy_1_0_5.likelihoods.mixed_noise.MixedNoise(likelihoods_list=likelihoods_list, noise_index=noise_index)
    likelihood = GPy_1_0_5.likelihoods.mixed_noise.MixedNoise(likelihoods_list=likelihoods_list)
    return likelihood
...
def ICM(input_dim, num_outputs, kernel, W_rank=1,W=None,kappa=None,name='ICM'):
    ...
    K = kernel.prod(GPy_1_0_5.kern.Coregionalize(1, num_outputs, active_dims=[input_dim], rank=W_rank,W=W,kappa=kappa,name='B'),name=name)
    return K
